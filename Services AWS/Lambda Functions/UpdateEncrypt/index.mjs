...
export const handler = async (event) => {
    const { id, name, address, phone } = JSON.parse(event.body);
    ...
    try {
        ...
        const [rows] = await connection.execute(
            'UPDATE customer SET name = ?, address = ?, phone = ?, iv_name = ?, iv_address = ?, iv_phone = ? WHERE id = ?', 
            [encryptedName.encryptedData, encryptedAddress.encryptedData, encryptedPhone.encryptedData, encryptedName.iv, encryptedAddress.iv, encryptedPhone.iv, id]
        );
        ...
        return {
            statusCode: 200,
            body: JSON.stringify({
                message: 'Datos actualizados y cifrados correctamente',
